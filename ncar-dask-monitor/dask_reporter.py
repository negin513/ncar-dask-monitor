...
def get_parser():
    ...
    group.add_argument("-s","--start_date",
                        type=str,
                        dest = 'start_date',
                        action = "store",
...
def main():
    ...
    print ('------------------------')
    print ("Memory usage summary of dask workers for period of ",args.start_date, '-', args.end_date)
    ...
    if (args.user=='all'):
        print ('hello!')
        grouped_dj = dj.groupby('User').agg({'Unused Mem (GB)': 'mean', 'Unused Mem (%)': 'mean','Elapsed (h)': 'mean',
            'Job ID': 'count'})
        dj_90 = grouped_dj[grouped_dj['Unused Mem (%)']>80]
        dj_90 ['Wasted Memory-Hour (GB-hr)'] = dj_90['Unused Mem (GB)']*dj_90['Elapsed (h)']*dj_90['Job ID']
        #dj_90=dj_90.rename(columns = {'Unused Mem':'Unused Mem (GB)'})

        print (dj_90.sort_values(by=['Wasted Memory-Hour (GB-hr)'], ascending=False)[0:10])
