...
class TestDaskMemUsage(unittest.TestCase):
    ...
    def test_missing_start_date_and_days(self):
        ...
        parser.error.assert_called_once_with("Either --start-date or -d/-days option must be provided.")
    ...
    def test_missing_end_date(self):
        ...
        parser.error.assert_called_once_with("End date is required if start date is provided.")
    ...
    def test_end_date_less_than_start_date(self):
        ...
        parser.error.assert_called_once_with("End date must be greater than start date.")
    ...
    def test_run_qhist(self):
        with patch('dask_mem_usage.QhistRunner') as mock_qhist_runner, \
             patch('dask_mem_usage.JobsSummary') as mock_jobs_summary:

            mock_qhist_runner.return_value.run_shell_code = MagicMock()
            mock_jobs_summary.return_value.dask_user_report = MagicMock()

            args = MagicMock(start_date='20220301', end_date='20220302', filename='log.txt', user='testuser')
            run_qhist(args)

            mock_qhist_runner.assert_called_once_with('20220301', '20220302', 'log.txt', 'testuser')
